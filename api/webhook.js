...
export default async function handler(req, res) {
  ...
  try {
    console.log("Raw list_id received:", JSON.stringify(list_id), "type:", typeof list_id);
    const parsedListId = parseInt(String(list_id || '').trim());
    let listId = (!isNaN(parsedListId) && parsedListId > 0) ? parsedListId : null;
    ...
    if (listId) {
      ...
      if (existing) {
        ...
        if (createRes.status === 200) {
          ...
          for (let attempt = 1; attempt <= 6 && !listId; attempt++) {
            await new Promise(r => setTimeout(r, attempt * 3000));
            const found = await findListByName(list_name);
            if (found) {
              listId = found.listId;
              console.log(`Found listId after wait (attempt ${attempt}):`, listId);
            }
          }
        }
      }
